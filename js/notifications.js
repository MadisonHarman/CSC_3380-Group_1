// ==============================
// Notifications System with Event Reminders
// New feature: Provides automatic notifications for upcoming events and reminders
// ==============================

// ---------- STATE MANAGEMENT ----------
function getNotifications() { // Retrieves all notifications from localStorage
  const st = loadState();
  return st.notifications || [];
}

function saveNotifications(list) { // Saves notifications to localStorage
  const st = loadState();
...
// ---------- UNIQUE ID ----------
function uuid() { // Generates a unique identifier for each notification
  return crypto.randomUUID();
}

// ---------- ADD NOTIFICATION ----------
function addNotification(message, date, time, options = {}) { // Creates and stores a new notification
...
// ---------- MARK AS READ ----------
function markAsRead(id) { // Marks a notification as read
...
// ---------- DELETE ----------
function deleteNotification(id) { // Removes a notification from the list
...
function sendBrowserNotification(title, body) { // Sends a browser notification if permission is granted
...
// ---------- PARSE TIME STRING ----------
function parseTimeToMinutes(timeStr) { // Converts a time string (HH:MM) to minutes since midnight
...
function checkUpcomingEvents() { // Checks for upcoming events and creates notifications
...
// ---------- BADGE UPDATE ----------
function updateNotificationBadge() { // Updates the notification badge on the bell icon
...
function renderNotifications() { // Renders the notification panel with all notifications
...
function cleanOldNotifications() { // Removes notifications older than 3 days
...
// ---------- INIT ----------
document.addEventListener("DOMContentLoaded", () => { // Initialize the notification system when the page loads
  const bell = document.getElementById("notif-bell");
