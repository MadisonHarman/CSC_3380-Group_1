...
function formatReminderTime(minutes) { // Formats reminder time in a human-readable format (e.g., "15m", "2h", "1d")
  if (!minutes) return "";
...
document.addEventListener("DOMContentLoaded", () => { // When the page is ready, initialize the agenda functionality.
  ...
  document.getElementById("add-agenda").addEventListener("click", () => { // For the 'Add to day' button, add event listeners.
    ...
    const reminder = document.getElementById("reminder-time").value; // New: Get the reminder time selected by the user
    ...
    upsertAgenda(dateISO, { 
      ...
      time, 
      tag: inferredTag,
      reminder: reminder ? parseInt(reminder) : null // New: Store the reminder time if set
    });
